...
// --------------------------- Geometric helpers ---------------------------
ob::StateSpacePtr makeDubinsSpace(double turningRadius, const ob::RealVectorBounds& xyBounds, bool symmetric) {
    auto space = std::make_shared<ob::DubinsStateSpace>(turningRadius, symmetric);
    space->as<ob::SE2StateSpace>()->setBounds(xyBounds);
    return space;
}

ob::StateSpacePtr makeReedsSheppSpace(double turningRadius, const ob::RealVectorBounds& xyBounds) {
    auto space = std::make_shared<ob::ReedsSheppStateSpace>(turningRadius);
    space->as<ob::SE2StateSpace>()->setBounds(xyBounds);
    return space;
}

std::shared_ptr<og::SimpleSetup> createDubinsSetup(double turningRadius, const ob::RealVectorBounds& xyBounds) {
    auto space = makeDubinsSpace(turningRadius, xyBounds, /*symmetric=*/false);
...
std::shared_ptr<og::SimpleSetup> createReedsSheppSetup(double turningRadius, const ob::RealVectorBounds& xyBounds) {
    auto space = makeReedsSheppSpace(turningRadius, xyBounds);
    auto ss = std::make_shared<og::SimpleSetup>(space);
    return ss;
}
...
og::PathGeometric dubinsSmooth(const std::shared_ptr<og::SimpleSetup>& ss,
                               ...
                               unsigned shortcutIters,
                               double maxTime,
                               bool reduceVerts) {
    og::PathGeometric out(in);
    og::PathSimplifier simpl(ss->getSpaceInformation());
    if (reduceVerts) simpl.reduceVertices(out);
    double t=0.0; unsigned it=0; const double dt= (shortcutIters? maxTime/shortcutIters : maxTime);
    while (it < shortcutIters && t < maxTime) {
        simpl.shortcutPath(out);
        ++it; t += dt;
    }
    return out;
}
...
og::PathGeometric dubinsSmoothFromControlPath(double turningRadius,
                                              ...
                                              double maxTime,
                                              bool reduceVerts) {
    auto ss = createDubinsSetup(turningRadius, xyBounds);
    ss->setStateValidityChecker(se2Validity);

    auto geoCtrl = ctrlPath.asGeometric();
    og::PathGeometric dubinsPath(ss->getSpaceInformation());
    for (const auto* st : geoCtrl.getStates()) {
        const auto* comp = st->as<ob::CompoundState>();
        const auto* se2  = comp->as<ob::SE2StateSpace::StateType>(0);
        ob::ScopedState<ob::SE2StateSpace> s2(ss->getSpaceInformation()->getStateSpace());
        s2->setX(se2->getX()); s2->setY(se2->getY()); s2->setYaw(se2->getYaw());
        dubinsPath.append(s2.get());
    }
    return dubinsSmooth(ss, dubinsPath, shortcutIters, maxTime, reduceVerts);
}
...
og::PathGeometric planReedsSheppSimple(const ob::RealVectorBounds& xyBounds,
    ...
    auto ss = createReedsSheppSetup(turningRadius, xyBounds);
    ss->setStateValidityChecker(validity);

    ob::ScopedState<ob::SE2StateSpace> s(ss->getSpaceInformation()->getStateSpace());
    ob::ScopedState<ob::SE2StateSpace> g(ss->getSpaceInformation()->getStateSpace());
    s->setX(startSE2[0]); s->setY(startSE2[1]); s->setYaw(startSE2[2]);
    g->setX(goalSE2[0]);  g->setY(goalSE2[1]);  g->setYaw(goalSE2[2]);
    ss->setStartAndGoalStates(s, g, 0.0);
    ...
    ss->setup();
    if (!ss->solve(timeSeconds)) {
        return og::PathGeometric(ss->getSpaceInformation());
    }
    auto path = ss->getSolutionPath();
    og::PathSimplifier simpl(ss->getSpaceInformation());
    simpl.reduceVertices(path);
    simpl.shortcutPath(path);
    return path;
}
